// service.go provides model catalog backend operations.
// internal/features/ai/model/app/model/service.go
package model
...
type ModelService struct {
	...
	db              *sql.DB
	appName         string
	fileSystem      aiinterfaces.FileSystem
	appDataResolver aiinterfaces.AppDataDirResolver
	seeder          aiinterfaces.ModelSeeder
}
...
func NewModelService(catalog ModelCatalogOperations, db *sql.DB, appName string, fileSystem aiinterfaces.FileSystem, appDataResolver aiinterfaces.AppDataDirResolver, seeder aiinterfaces.ModelSeeder) *ModelService {
...
func (s *ModelService) ImportModels(_ context.Context, request aiinterfaces.ImportModelsRequest) error {
	...
	if s.fileSystem == nil {
		return fmt.Errorf("backend service: file system not configured")
	}
	if s.seeder == nil {
		return fmt.Errorf("backend service: model seeder not configured")
	}
	...
	payload, err := s.fileSystem.ReadFile(path)
	if err != nil {
		return fmt.Errorf("backend service: read model file: %w", err)
	}

	if err := s.seeder.SeedModels(s.db, payload); err != nil {
...
func (s *ModelService) SyncModels(ctx context.Context) (aiinterfaces.SyncModelsResult, error) {
	...
	if s.appDataResolver == nil {
		return aiinterfaces.SyncModelsResult{}, fmt.Errorf("backend service: app data resolver not configured")
	}
	if s.fileSystem == nil {
		return aiinterfaces.SyncModelsResult{}, fmt.Errorf("backend service: file system not configured")
	}

	appDataDir, err := s.appDataResolver.ResolveAppDataDir(s.appName)
	if err != nil {
	...
	exists, err := s.fileSystem.FileExists(customModelsPath)
	if err != nil {
