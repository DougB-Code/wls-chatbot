...
// internal/adapters/chatrepo/sqlite_test.go
package chatrepo
...
func TestRepositoryCreateAndGetRoundTrip(t *testing.T) {
	...
	conv := &chatdomain.Conversation{
		ID:    "conv-1",
		Title: "Launch Plan",
		Settings: chatdomain.ConversationSettings{
			Provider:     "openai",
			Model:        "gpt-4o",
			Temperature:  0.2,
			MaxTokens:    2048,
		...
		Messages: []*chatdomain.Message{
			{
				...
				Blocks: []chatdomain.Block{
					{Type: chatdomain.BlockTypeText, Content: "Hello"},
				},
				Timestamp: 10,
			},
			{
				...
				Blocks: []chatdomain.Block{
					{Type: chatdomain.BlockTypeText, Content: "Hi there"},
				},
				Timestamp: 11,
				Metadata: &chatdomain.MessageMetadata{
					Provider:     "openai",
					Model:        "gpt-4o",
					TokensIn:     12,
					TokensOut:    34,
...
func TestRepositoryUpdateReplacesMessages(t *testing.T) {
		...
		Messages: []*chatdomain.Message{
			{
				...
				Role:           chatdomain.RoleUser,
				Blocks: []chatdomain.Block{
					{Type: chatdomain.BlockTypeText, Content: "Old"},
				},
				Timestamp: 1,
			},
		},
	...
	conv.Title = "Updated"
	conv.UpdatedAt = 3
	conv.Messages = []*chatdomain.Message{
		{
			ID:             "msg-new",
			ConversationID: "conv-2",
			Role:           chatdomain.RoleAssistant,
			Blocks: []chatdomain.Block{
				{Type: chatdomain.BlockTypeError, Content: "provider unavailable"},
			},
			Timestamp: 3,
			Metadata: &chatdomain.MessageMetadata{
				Provider:     "openai",
