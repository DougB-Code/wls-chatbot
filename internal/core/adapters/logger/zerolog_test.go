...
func TestCoreAdapter(t *testing.T) {
	...
	t.Run("Info with fields", func(t *testing.T) {
		adapter.Info("test info", ports.LogField{Key: "user_id", Value: "123"})
	...
	t.Run("Error with error object", func(t *testing.T) {
		...
		adapter.Error("operation failed", testErr, ports.LogField{Key: "attempt", Value: "1"})
	...
	t.Run("Debug", func(t *testing.T) {
		adapter.Debug("debugging", ports.LogField{Key: "foo", Value: "bar"})
